...
const QUICK = [
  ...
  'Tips for walking home at night',
...
export default function AIAssistantScreen() {
  ...
  const send = async (text = input) => {
    ...
    setLoading(true);
    try {
      const r = await getAI(text.trim(), msgs);
      setMsgs(p => [...p, {
      ...
      setMsgs(p => [...p, {
        id: (Date.now() + 1).toString(),
        role: 'assistant',
        text: "I'm temporarily offline. If this is an emergency, use the SOS button on the home screen.",
        time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
      }]);
    }
    setLoading(false);
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: COLORS.bg }} edges={['top']}>
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'} keyboardVerticalOffset={Platform.OS === 'ios' ? 0 : 20}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12, paddingHorizontal: 24, paddingVertical: 16, borderBottomWidth: 1, borderBottomColor: COLORS.border }}>
          <View style={{ width: 44, height: 44, borderRadius: 22, backgroundColor: COLORS.purpleDim, borderWidth: 1, borderColor: COLORS.purple + '40', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <ScrollView ref={ref} style={{ flex: 1 }} contentContainerStyle={{ padding: 24, gap: 12 }} showsVerticalScrollIndicator={false}>
          ...
          {loading && (
            <View style={{ backgroundColor: COLORS.bgCard, borderRadius: 16, padding: 16, alignSelf: 'flex-start', borderWidth: 1, borderColor: COLORS.border }}>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'flex-end', gap: 8, padding: 16, paddingBottom: 24, borderTopWidth: 1, borderTopColor: COLORS.border }}>
          <TextInput
            style={{ flex: 1, backgroundColor: COLORS.bgCard, borderWidth: 1, borderColor: COLORS.border, borderRadius: 16, padding: 16, color: COLORS.text, fontSize: 15, maxHeight: 120 }}
            value={input}
          ...
          />
          <TouchableOpacity
            style={{ width: 44, height: 44, borderRadius: 22, backgroundColor: input.trim() ? COLORS.purple : COLORS.bgCard, alignItems: 'center', justifyContent: 'center', borderWidth: input.trim() ? 0 : 1, borderColor: COLORS.border }}
            onPress={() => send()}
